    ...
    try {
        ...
        switch (action) {
            ...
            case 'getDashboardData': {
                ...
                const { schoolId, selectedDate } = payload;
                if (!schoolId) {
                    return response.status(400).json({ error: 'School ID required' });
                }
                
                const { rows: schoolInfoRows } = await sql`
                    WITH all_student_data AS (
                        SELECT DISTINCT ON (sbc.key) sbc.key as class_name, sbc.value as class_data
                        FROM absensi_data ad, jsonb_each(ad.students_by_class) sbc
                        WHERE ad.school_id = ${schoolId}
                        ORDER BY sbc.key, ad.last_updated DESC
                    ),
                    class_stats AS (
                        SELECT
                            class_name,
                            jsonb_array_length(class_data -> 'students') as student_count
                        FROM all_student_data
                    )
                    SELECT
                        (SELECT COALESCE(SUM(student_count), 0) FROM class_stats) as "totalStudents",
                        (SELECT COALESCE(jsonb_agg(class_name ORDER BY class_name), '[]'::jsonb) FROM class_stats) as "allClasses",
                        (SELECT COALESCE(jsonb_object_agg(class_name, student_count), '{}'::jsonb) FROM class_stats) as "studentsPerClass";
                `;
                const schoolInfo = {
                    totalStudents: schoolInfoRows[0]?.totalStudents || 0,
                    allClasses: schoolInfoRows[0]?.allClasses || [],
                    studentsPerClass: schoolInfoRows[0]?.studentsPerClass || {}
                };

                const { rows: dailyAbsentRows } = await sql`
                    WITH unnested_logs AS (
                      SELECT log_obj, u.name as user_name
                      FROM absensi_data ad CROSS JOIN jsonb_array_elements(ad.saved_logs) as log_obj
                      JOIN users u on u.email = ad.user_email
                      WHERE ad.school_id = ${schoolId} AND log_obj ->> 'date' = ${selectedDate}
                    ),
                    absent_students AS (
                      SELECT log_obj ->> 'class' as class, user_name, att.key as student_name, att.value as status
                      FROM unnested_logs CROSS JOIN jsonb_each_text(log_obj -> 'attendance') as att
                      WHERE att.value <> 'H'
                    )
                    SELECT class, user_name as "teacherName",
                      jsonb_agg(jsonb_build_object('name', student_name, 'status', status) ORDER BY student_name) as students
                    FROM absent_students GROUP BY class, user_name ORDER BY class;
                `;
                ...
                dailyAbsentRows.forEach(row => {
                    ...
                    row.students.forEach(student => {
                        if (absenceCounts[student.status] !== undefined) absenceCounts[student.status]++;
                    });
                });
                const totalAbsent = absenceCounts.S + absenceCounts.I + absenceCounts.A;
                const totalPresent = Math.max(0, schoolInfo.totalStudents - totalAbsent);
                
                const todayForYear = new Date(selectedDate + 'T00:00:00');
                let yearStart = new Date(todayForYear.getFullYear(), 6, 1); // July 1st
                if (todayForYear.getMonth() < 6) { yearStart.setFullYear(todayForYear.getFullYear() - 1); }
                const yearEnd = new Date(yearStart.getFullYear() + 1, 5, 30); // June 30th next year

                const { rows: allLogsRows } = await sql`
                    SELECT log_obj as log
                    FROM absensi_data, jsonb_array_elements(saved_logs) as log_obj
                    WHERE school_id = ${schoolId} AND (log_obj->>'date')::date BETWEEN ${yearStart.toISOString().split('T')[0]} AND ${yearEnd.toISOString().split('T')[0]};
                `;
                const allLogsForYear = allLogsRows.map(r => r.log);

                return response.status(200).json({
                    reportData: { summaryStats: { totalStudents: schoolInfo.totalStudents, totalPresent, ...absenceCounts }, absentStudentsByClass },
